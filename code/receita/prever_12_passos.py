"""
Previsão de 12 passos à frente de cada série de receita, com o melhor modelo por
série entre os 13 avaliados (individuais, locais — pooled e LGBM-cluster — e
globais, incluindo o Chronos). O vencedor é lido da comparacao_unificada (menor
RMSE de teste) e só a previsão futura dele é gerada.

Saídas:
  results/previsoes_12_passos_receita.csv / .xlsx (tabela)
  results/forecasts_receita/forecast_<cod>_<mod>.png + _grade_NN.png (paginado)
"""
...
OUT = RESULTS_DIR / "forecasts_receita"; OUT.mkdir(parents=True, exist_ok=True)
...
RMSE_COL = {"Ind":"Ind_RMSE","Local_Ridge":"Pooled_Ridge_RMSE","Local_SVR":"Pooled_SVR_RMSE",
 "Local_MLP":"Pooled_MLP_RMSE","Local_LGBM":"LGBM_Cluster_RMSE",
 "Glob_Ridge":"Global_Ridge_RMSE","Glob_RF":"Global_RF_RMSE","Glob_MLP":"Global_MLP_RMSE",
 "Glob_LGBM":"Global_LGBM_RMSE","Glob_Chronos":"Global_Chronos_RMSE"}
...
print(f"{len(prep)} series. Treinando modelos globais (LightGBM, Ridge, RF, MLP, Chronos)...")
...
def _global_fit(cod, p, nl, predict_norm, mean, std, sid):
...
def h_glob_lgbm(cod,p):
    nl=N_LAGS_GLOBAL; mean,std=norm_params[cod]; sid=int(label_enc.transform([cod])[0])
    fit=_global_fit(cod,p,nl, lambda Xs: mod_lgbm.predict(_df(Xs,nl)), mean,std,sid)
...
def h_glob_sklearn(nome,cod,p):
    ...
    fit=_global_fit(cod,p,nl, lambda Xs: modelo.predict(Xs if usa_array else _df(Xs,nl)), mean,std,sid)
...
def _cluster_membros(cod):
    cid=cluster_de.get(cod)
    return [c for c in cluster_de if cluster_de[c]==cid and c in series_data]

def h_local(nome,cod,p):
    nl=_nlags(p); membros=_cluster_membros(cod)
...
def h_local_lgbm(cod,p):
    nl=N_LAGS_GLOBAL; membros=_cluster_membros(cod)
    sd={c:series_data[c] for c in membros}
    Xc,yc,npar,le=construir_dataset_global(sd,nl)
    modelo,_=treinar_lgbm_global(Xc,yc)
    mean,std=npar[cod]; sid=int(le.transform([cod])[0])
    fit=_global_fit(cod,p,nl, lambda Xs: modelo.predict(_df(Xs,nl)), mean,std,sid)
    tp=np.asarray(prever_rolling_lgbm(modelo,p["tv"],p["tsv"],p["tidx"],p["tesidx"],nl,mean,std,sid),float)
    todos=p["sp"].values.astype(float); tidx=p["tidx"].append(p["tesidx"])
    fut=np.asarray(prever_rolling_lgbm(modelo,todos,np.zeros(HORIZONTE),tidx,fut_pidx,nl,mean,std,sid),float)
    return fit,tp,fut
...
LBL={"Glob_LGBM":"LightGBM global","Glob_RF":"RF global","Glob_MLP":"MLP global",
     ...
     "Local_SVR":"SVR local","Local_MLP":"MLP local","Local_LGBM":"LightGBM local"}
...
for _, row in udf.sort_values("Codigo").iterrows():
    ...
    try:
        ...
        elif mod=="Local_LGBM":    fit,tp,fut=h_local_lgbm(cod,p)
        ...
        fig.savefig(OUT/f"forecast_{cod}_{tag}.png",dpi=130,bbox_inches="tight"); plt.close(fig)
        paineis.append((cod,lbl,all_d,obs,td,fit_o,ed,tp_o,fut_o))
        print(f"[OK] {cod}  {lbl}")
    except Exception as e:
        print(f"[FALHA] {cod} {mod}: {e}")

# grades paginadas (24 paineis por imagem)
PP=24; nc=4
for g in range(0,len(paineis),PP):
    bloco=paineis[g:g+PP]; nr=int(np.ceil(len(bloco)/nc))
    fig,axes=plt.subplots(nr,nc,figsize=(nc*4.2,nr*2.4)); axes=np.atleast_1d(axes).ravel()
    for ax,(cod,lbl,all_d,obs,td,fit_o,ed,tp_o,fut_o) in zip(axes,bloco):
        ax.plot(all_d,obs,color="black",lw=1.0); ax.plot(td,fit_o,color="#1f77b4",lw=0.8,ls="--")
        ax.plot(ed,tp_o,color="#ff7f0e",lw=1.2); ax.plot(fut_dates,fut_o,color="#d62728",lw=1.2)
        ax.axvline(ed[0],color="gray",ls=":",alpha=0.6); ax.axvline(fut_dates[0],color="gray",ls=":",alpha=0.6)
        ax.set_title(f"{cod} [{lbl}]",fontsize=7); ax.yaxis.set_major_formatter(FuncFormatter(fmt))
        ax.tick_params(labelsize=5); ax.grid(alpha=0.25,ls="--")
    for ax in axes[len(bloco):]: ax.axis("off")
    fig.tight_layout(); fig.savefig(OUT/f"_grade_{g//PP+1:02d}.png",dpi=120,bbox_inches="tight"); plt.close(fig)

out=pd.DataFrame(linhas)
out.to_csv(RESULTS_DIR/"previsoes_12_passos_receita.csv",index=False,encoding="utf-8-sig")
with pd.ExcelWriter(RESULTS_DIR/"previsoes_12_passos_receita.xlsx",engine="openpyxl") as w:
