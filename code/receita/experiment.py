"""
Orquestrador do experimento individual (um modelo por série de receita): ARIMA,
LR (Ridge), SVR e MLP.
...
# Checkpoint
# Altere a versao sempre que o protocolo de avaliacao mudar (invalida checkpoints
# antigos automaticamente).
_CHECKPOINT_VERSION = "v1_individual_multistep"
_CHECKPOINT_PATH    = RESULTS_DIR / "checkpoint_individual.pkl"


def _salvar_checkpoint(
    registros_met   : list,
    registros_params: list,
    registros_prev  : list,
    timestamp       : str,
) -> None:
    """Persiste os registros ja processados em disco."""
...
def _carregar_checkpoint() -> dict | None:
    """
    Carrega checkpoint existente e valida o schema.
    ...
    try:
        ...
        if versao != _CHECKPOINT_VERSION:
            ...
            return None
        # Conta series unicas ja concluidas (ruido-branco + modeladas)
        n = len(data.get("concluidos", set()))
        logger.info(
            "Checkpoint encontrado: %d series ja processadas (schema=%s). Retomando...",
            n, versao,
        )
        return data
...
def rodar_experimento(
    ...
    """
    Executa o experimento completo de previsão de despesas.

    Suporta retomada via checkpoint: se interrompido durante o loop por série,
    recarrega o progresso salvo e pula as séries já processadas (evita
    re-rodar o grid search de ARIMA/SVR/MLP para séries concluídas).
    ...
    if ckpt is not None:
        ...
        logger.info("Retomando: %d series ja processadas.", len(concluidos))
    else:
    ...
    for idx, (codigo, serie_original) in enumerate(series.items(), 1):
        ...
        # Pula série já processada (retomada de checkpoint)
        if str(codigo) in concluidos:
            logger.debug("[%d/%d] %s — ja processada, pulando.", idx, total, codigo)
            continue
        ...
        # Checkpoint após série concluída com sucesso
        concluidos.add(str(codigo))
        _salvar_checkpoint(registros_met, registros_params, registros_prev, timestamp)
    ...
    # Remove checkpoint — experimento concluído com sucesso
...
if __name__ == "__main__":
    ...
    print("\n-- Previsões Finais ----------------------------------------------")
    prev_cols = [c for c in resultados["prev_df"].columns if c.startswith("Prev_")]
    cols_show = ["Codigo", "Melhor_Modelo", "RMSE_Teste"] + prev_cols[:2]
    print(
        resultados["prev_df"][cols_show].to_string(index=False)
    )
