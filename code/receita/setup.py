"""
Configuração inicial do projeto. Rode uma vez depois de clonar:
...
def checar_dados():
    ...
    if CAMINHO_PARQUET.exists():
        ...
        print(f"     Copie o arquivo antes de rodar experiment.py")


def checar_weasyprint():
    try:
        import weasyprint  # noqa
        print(f"{OK} weasyprint (exportação PDF disponível)")
    except ImportError:
        print(f"{WARN} weasyprint não instalado — exportação PDF desabilitada.")
        print(f"     Para instalar: pip install weasyprint")


# Main
if __name__ == "__main__":
    print("\n" + "─" * 55)
    print("  SETUP — Previsão de Séries Temporais de Receita")
    print("─" * 55)
