"""
Estudo de ablação do experimento, isolando o efeito de dois componentes:

  1. Número de lags (3, 6, 12): mais lags sempre ajuda? qual o ideal por modelo?
  2. Tamanho do conjunto de teste (3, 6, 12 meses): o ranking dos modelos muda quando se
     avalia em horizontes maiores?

Em cada configuração, os quatro modelos (ARIMA, LR, SVR, MLP) são avaliados com a
mesma previsão recursiva do experimento principal, mantendo o resto fixo.
...
def ablation_lags(caminho=CAMINHO_PARQUET, verbose=True) -> pd.DataFrame:
    ...
    pd.DataFrame com colunas:
        Codigo, Grupo_Sinal, N_Total, N_Treino, N_Holdout,
        N_Lags, Modelo, RMSE, MAE, MAPE
    """
    ...
    for idx, (codigo, serie_original) in enumerate(series.items(), 1):
        ...
        # Conjunto de teste adaptativo fixo para este ablation
        ...
        # Teste de ruído branco — séries sem estrutura são puladas
...
# 2. Ablation: tamanho do conjunto de teste
...
def ablation_holdout(caminho=CAMINHO_PARQUET, verbose=True) -> pd.DataFrame:
    ...
    for idx, (codigo, serie_original) in enumerate(series.items(), 1):
        ...
        for n_holdout in ABLATION_HOLDOUTS:
            ...
            # Lags selecionados pelo ACF do treino desta configuração
            acf_info = analisar_serie(treino)
            if acf_info["e_ruido_branco"]:
                continue   # pula apenas este conjunto de teste; outros podem ser válidos
